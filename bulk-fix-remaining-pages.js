...
const pages = [
  ...
  { 
    file: 'traces-dashboard.html', 
    activeCategory: '관찰성', 
    activeItem: '🔗 트레이스 대시보드' 
  },
  { 
...
pages.forEach(pageInfo => {
  ...
  try {
    ...
    if (!content.includes('Dropdown 메뉴 완전 제어')) {
      ...
      if (styleStart !== -1) {
        const afterStyleTag = content.indexOf('>', styleStart) + 1;
        const firstLineEnd = content.indexOf('\\n', afterStyleTag);
        content = content.slice(0, firstLineEnd) + dropdownCSS + content.slice(firstLineEnd);
      }
    }
    
    // 2. Tailwind CSS CDN으로 변경 (필요한 경우)
    if (content.includes('./css/tailwind.css')) {
      content = content.replace(
        /<link href="\.\/css\/tailwind\.css" rel="stylesheet">/g,
        '<script src="https://cdn.tailwindcss.com"></script>'
      );
    }
    
    // 3. Navigation 구조 교체
    const navStartRegex = /<!--\s*Navigation\s*-->\s*<nav[^>]*>/;
    const navEndRegex = /<\/nav>/;
    
    const navStartMatch = content.match(navStartRegex);
    if (navStartMatch) {
      const navStart = navStartMatch.index;
      const afterNavStart = navStart + navStartMatch[0].length;
      const navEndMatch = content.slice(afterNavStart).match(navEndRegex);
      
      if (navEndMatch) {
        const navEnd = afterNavStart + navEndMatch.index + navEndMatch[0].length;
